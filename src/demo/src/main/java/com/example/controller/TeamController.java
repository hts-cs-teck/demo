...
public class TeamController {
	...
	private List<TeamDto> getSerachTeamList(String spid, String sname) {

		List<Team> steamList = new ArrayList<>();
		List<TeamDto> steamDtoList = new ArrayList<>();

		if (StringUtil.isNullOrEmpty(spid) && StringUtil.isNullOrEmpty(sname)) {
			// 検索条件指定なしの場合
			// 全件取得
			steamList = teamService.findAll();
			if (StringUtil.isNullOrEmpty(sname)) {
			} else {
				// チーム名のみ指定の場合
				// チーム名で検索
				steamList = teamService.findByNameLike(sname);
			}
			...
			// チームが選択されている場合
			steamList = teamService.findById(Long.parseLong(spid));
		}
	...
	public String teamSearch(Model model, TeamListModel teamListModel){
		try {
			...
			model.addAttribute("steamList2", getSerachTeamList(teamListModel.getSpid(), teamListModel.getSname()));
	...
	public String deleteTeamController(Model model, TeamModel teamModel)  {
		ERROR : try {
			...
			model.addAttribute("steamList", getSerachTeamList());
			model.addAttribute("steamList2", getSerachTeamList(teamModel.getRspid(), teamModel.getRsname()));
